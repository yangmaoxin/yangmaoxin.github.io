...
#!/bin/bash
# author：yangmaoxin
# website：https://github.com/yangmaoxin
# slogan：人总需要点什么来坚持下去。或是希望，或是信仰。我和这个世界不熟，但我怀有期待。
#--------------------------------------------
...
EOF
  ...
  git init
  git config user.name "yangmaoxin"
  ...
  # Gitee Pages
  git push --force --quiet "https://yangmaoxin:${GITEE_TOKEN}@${GITEE_REF}" master:master
  ...
  # Gitee Pages
  git push --quiet "https://moxinn:${GITEE_TOKEN}@${GITEE_REF}" master:master --tags
}
